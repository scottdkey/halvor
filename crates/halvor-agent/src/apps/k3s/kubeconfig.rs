...
//! - **Join operations** (in join.rs) use **IP addresses** because systemd services can't resolve Tailscale hostnames
//! - **Kubeconfig files** (this module) use **FQDNs** (hostnames like `frigg.bombay-pinecone.ts.net`) for:
...
/// Fetch kubeconfig content from environment variable or primary node
...
pub fn fetch_kubeconfig_content(primary_hostname: &str, config: &EnvConfig) -> Result<String> {
    ...
    // Helper function to process kubeconfig content - replaces localhost/IP with Tailscale hostname
    let process_kubeconfig = |mut kubeconfig: String, hostname: &str| -> Result<String> {
        // Get Tailscale IP/hostname for the node to replace 127.0.0.1 and IPs
        ...
        // Prefer hostname over IP - construct if lookup failed
        let server_address = if let Some(ref ts_hostname) = tailscale_hostname {
            ts_hostname.as_str()
        } else if !config._tailnet_base.is_empty() {
            let tailnet_base = &config._tailnet_base;
            // Construct hostname from primary_hostname and tailnet_base
            let constructed = if hostname.contains('.') {
                hostname.to_string()
            } else {
                format!("{}.{}", hostname, tailnet_base)
            };
            println!("  ✓ Constructed Tailscale hostname: {}", constructed);
            // Replace all occurrences
            kubeconfig = kubeconfig.replace("127.0.0.1", &constructed);
            kubeconfig = kubeconfig.replace("localhost", &constructed);
            // Also replace Tailscale IPs with hostname
            kubeconfig = kubeconfig.replace(&tailscale_ip, &constructed);
        ...
        // Also replace Tailscale IPs with hostname if we have one
        if tailscale_hostname.is_some() {
            kubeconfig = kubeconfig.replace(&tailscale_ip, server_address);
        }
    ...
    if std::path::Path::new("/etc/rancher/k3s/k3s.yaml").exists() {
        ...
        if is_local_primary {
                    ...
                    if let Ok(status_output) = tailscale_status {
                        if status_output.status.success() {
                                ...
                                serde_json::from_str::<serde_json::Value>(&status_str)
                            {
                                ...
                                if let Some(ts_hostname) = status_json
                                    .get("Self")
                                ...
                                } else if let Some(ts_ip) = status_json
                                    .get("Self")
            ...
            println!(
                "  ⚠ Local K3s found but primary is different node ({}), will fetch from primary",
                primary_hostname
            );
        }
    }

    // Next, try to get from KUBE_CONFIG environment variable
    ...
    if let Some(exec) = primary_exec {
        if !exec.is_local() {
            ...
            match exec.execute_shell("sudo cat /etc/rancher/k3s/k3s.yaml") {
                ...
                _ => {
                    println!(
                        "  ⚠ Could not fetch kubeconfig from primary node (need sudo access or K3s not installed)"
                    );
                }
            }
        }
    ...
    anyhow::bail!(
        "Kubeconfig not found. Tried:\n\
         ...
         2. KUBE_CONFIG environment variable\n\
         3. Fetching from primary node ({})\n\
         \n\
         Please either:\n\
         - Ensure K3s is installed on the primary node, or\n\
         - Set KUBE_CONFIG environment variable with the kubeconfig content",
        primary_hostname
    )
}
...
fn process_kubeconfig_for_primary_internal(
        ...
        && !cleaned_content.trim_start().starts_with("kind:")
    {
        ...
        println!(
            "  ⚠ Kubeconfig doesn't start with 'apiVersion' or 'kind:', may need additional processing"
        );

        // Try to find YAML content if it's embedded in something else
        if let Some(api_start) = cleaned_content.find("apiVersion") {
            println!(
                "  Found 'apiVersion' at position {}, extracting from there",
                api_start
            );
    ...
    let docs = match YamlLoader::load_from_str(&cleaned_content) {
        ...
        Err(e) => {
            ...
            let char_at_70 = if cleaned_content.len() > 70 {
                format!(
                    "Character at position 70: '{}' (0x{:02x})",
            ...
            let first_line_display = if first_line.len() > 100 {
                    ...
                    Err(fetch_err) => {
                        anyhow::bail!(
                            "Failed to parse kubeconfig YAML from KUBE_CONFIG environment variable, and also failed to fetch from {}.\n\
                             ...
                             Fetch error: {}\n\
                             \n\
                             The kubeconfig from environment variable may be incorrectly formatted.\n\
                             Common issues:\n\
                             ...
                             \n\
                             Solution:\n\
                             Ensure KUBE_CONFIG environment variable is properly formatted.\n\
    ...
    // Try to get Tailscale hostname for the primary node
    // Prefer hostname over IP for better reliability and certificate validation
    let primary_server_address = {
        let exec = Executor::new(primary_hostname, config).ok();
        if let Some(exec) = exec {
            // Try to get Tailscale hostname first
            if let Ok(Some(ts_hostname)) = tailscale::get_tailscale_hostname_remote(&exec) {
                println!("  ✓ Using Tailscale hostname: {}", ts_hostname);
                ts_hostname
            } else {
                // Fall back to getting Tailscale IP if hostname lookup fails
                if let Ok(ts_ip) =
                    tailscale::get_tailscale_ip_with_fallback(&exec, primary_hostname, config)
                {
                    // Only use IP if existing server is also an IP (100.x.x.x)
                    // Otherwise, try to construct hostname from primary_hostname
                    if existing_server_host.contains("100.") {
                        println!("  ⚠ Using Tailscale IP (hostname lookup failed): {}", ts_ip);
                        ts_ip
                    } else {
                        // Try to construct hostname from primary_hostname
                        // If primary_hostname is like "frigg", construct "frigg.bombay-pinecone.ts.net"
                        if !config._tailnet_base.is_empty() {
                            let tailnet_base = &config._tailnet_base;
                            let constructed_hostname = if primary_hostname.contains('.') {
                                primary_hostname.to_string()
                            } else {
                                format!("{}.{}", primary_hostname, tailnet_base)
                            };
                            println!(
                                "  ✓ Constructed Tailscale hostname: {}",
                                constructed_hostname
                            );
                            constructed_hostname
                        } else {
                            println!("  ⚠ Using Tailscale IP (no tailnet base): {}", ts_ip);
                            ts_ip
                        }
                    }
                } else {
                    // All lookups failed, extract from existing server
                    let existing_addr = existing_server_host
                        .strip_prefix("https://")
                        .or_else(|| existing_server_host.strip_prefix("http://"))
                        .and_then(|s| s.split(':').next())
                        .unwrap_or(existing_server_host);
                    println!(
                        "  ⚠ Could not get Tailscale address, using existing: {}",
                        existing_addr
                    );
            ...
            // Could not create executor, try to construct hostname from primary_hostname
            if !config._tailnet_base.is_empty() {
                let tailnet_base = &config._tailnet_base;
                let constructed_hostname = if primary_hostname.contains('.') {
                    primary_hostname.to_string()
                ...
                println!(
                    "  ✓ Constructed Tailscale hostname from config: {}",
                    constructed_hostname
                );
                constructed_hostname
            } else {
                // Extract from existing server as last resort
                let existing_addr = existing_server_host
                    .strip_prefix("https://")
                    .or_else(|| existing_server_host.strip_prefix("http://"))
                    .and_then(|s| s.split(':').next())
                    .unwrap_or(existing_server_host);
                println!("  ⚠ Using existing server address: {}", existing_addr);
                existing_addr.to_string()
            }
        }
    };

    let primary_server_url = format!("https://{}:6443", primary_server_address);
    println!(
        "  Detected server from kubeconfig: {}",
    ...
    println!(
        "  Found {} server URL(s) to replace",
        servers_to_replace.len()
    );
    ...
    for current_server in &servers_to_replace {
        ...
        if let Some(host_part) = current_server
            .strip_prefix("https://")
            .or_else(|| current_server.strip_prefix("http://"))
        {
            if let Some(hostname) = host_part.split(':').next() {
                ...
                if processed.contains(hostname) && hostname != primary_server_address {
                    ...
                    processed = processed
                        .replace(&format!("https://{}:6443", hostname), &primary_server_url);
                    processed = processed
                        .replace(&format!("http://{}:6443", hostname), &primary_server_url);
                    processed = processed.replace(
                        &format!("{}:6443", hostname),
                        &format!("{}:6443", primary_server_address),
                    );
                ...
                for (idx, cluster) in clusters.iter().enumerate() {
                    if let Some(server) = cluster["cluster"]["server"].as_str() {
                        if server == &primary_server_url {
                            ...
                            println!(
                                "  ⚠ Warning: Cluster {} still has server: {} (expected: {})",
                                idx, server, primary_server_url
                            );
...
pub fn extract_server_and_token_from_kubeconfig(
    ...
    let token = doc["users"][0]["user"]["token"]
        ...
        .ok_or_else(|| {
            anyhow::anyhow!("No token or client-certificate-data found in kubeconfig users")
        })?
        .to_string();
