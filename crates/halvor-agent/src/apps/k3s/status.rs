        ...
        if !env_token.trim().is_empty() {
            ...
            if token_result.is_ok() {
                if let Ok(token_content) = exec.read_file(token_tmp) {
                    let parsed_token = crate::apps::k3s::utils::parse_node_token(&token_content);
                    if !parsed_token.is_empty() {
                        parsed_token
                    } else {
                        anyhow::bail!(
                            "Could not retrieve cluster token. K3S_TOKEN is empty and token file is empty. Please set K3S_TOKEN environment variable or ensure the cluster is initialized."
                        )
                    }
                } else {
                    anyhow::bail!(
                        "Could not retrieve cluster token. K3S_TOKEN is empty and could not read token file. Please set K3S_TOKEN environment variable or ensure the cluster is initialized."
                    )
                }
            } else {
                anyhow::bail!(
                    "Could not retrieve cluster token. K3S_TOKEN is empty and could not access token file. Please set K3S_TOKEN environment variable or ensure the cluster is initialized."
                )
            }
        }
    } else {
        ...
        if token_result.is_ok() {
            if let Ok(token_content) = exec.read_file(token_tmp) {
                ...
                if !parsed_token.is_empty() {
                    ...
                    anyhow::bail!(
                        "Could not retrieve cluster token. Token file is empty and K3S_TOKEN is not set. Please set K3S_TOKEN environment variable or ensure the cluster is initialized."
                    )
                }
            } else {
                anyhow::bail!(
                    "Could not retrieve cluster token. Could not read token file and K3S_TOKEN is not set. Please set K3S_TOKEN environment variable or ensure the cluster is initialized."
                )
            }
        } else {
            anyhow::bail!(
                "Could not retrieve cluster token. Could not access token file and K3S_TOKEN is not set. Please set K3S_TOKEN environment variable or ensure the cluster is initialized."
            )
        }
    };
