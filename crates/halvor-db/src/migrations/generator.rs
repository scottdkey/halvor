...
pub fn generate_migrations_cli() -> Result<()> {
    let migrations_dir = Path::new("crates/halvor-db/src/migrations");
        ...
        for entry in entries {
            if let Ok(entry) = entry {
                ...
                if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                    // Skip mod.rs and generated.rs
                    if file_name == "mod.rs" || file_name == "generated.rs" {
                        continue;
                    }
    ...
    for (version, name, mod_name, file_name) in &migrations {
        mod_declarations.push_str(&format!(
            "mod {} {{\n    include!(concat!(env!(\"CARGO_MANIFEST_DIR\"), \"/src/migrations/{}\"));\n}}\n",
            mod_name, file_name
        ));
...
"#,
    ...
    // Write to OUT_DIR if available (build.rs context), otherwise to src/migrations/generated.rs
    let output_path = if let Ok(out_dir) = env::var("OUT_DIR") {
        ...
        PathBuf::from("crates/halvor-db/src/migrations").join("generated.rs")
    };
