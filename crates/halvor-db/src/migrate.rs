...
pub fn migrate_down() -> Result<()> {
    ...
    crate::migrations::migrate_down(&conn)?;
...
pub fn migrate_list() -> Result<()> {
    ...
    let status = crate::migrations::get_migration_status(&conn)?;
    let current_version = crate::migrations::get_current_migration_version(&conn)?;
...
fn rollback_to_version(conn: &Connection, target_version: u32) -> Result<()> {
    let mut current_version = crate::migrations::get_current_migration_version(conn)?;
    ...
    while current_version >= target_version {
        ...
        crate::migrations::migrate_down(conn)?;
        current_version = crate::migrations::get_current_migration_version(conn)?;
    }
