...
use super::api_def::{ApiDefinition, ApiEndpoint};
...
fn generate_kotlin_client(endpoints: &[ApiEndpoint], output_base: &PathBuf) -> Result<()> {
    ...
    // Client class
    code.push_str(
        "class HalvorApiClient(private val baseUrl: String = \"http://localhost:8080\") {\n",
    );
    ...
    for endpoint in endpoints {
        ...
        code.push_str("        }\n\n");
        code.push_str(&format!(
            "        val response: ApiResponse<{}> = json.decodeFromString(\n",
            return_type
        ));
...
fn generate_swift_client(endpoints: &[ApiEndpoint], output_base: &PathBuf) -> Result<()> {
    ...
    for endpoint in endpoints {
        ...
        code.push_str(
            "            throw HalvorApiError.apiError(apiResponse.error ?? \"Unknown error\")\n",
        );
        code.push_str("        }\n\n");
