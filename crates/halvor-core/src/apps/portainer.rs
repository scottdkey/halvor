...
pub fn install_host<E: CommandExecutor>(exec: &E, edition: PortainerEdition) -> Result<()> {
    ...
    if let Ok(containers) = halvor_docker::list_containers(exec) {
        for container in &containers {
            if container == "portainer" || container == "portainer_agent" {
                halvor_docker::stop_and_remove_container(exec, container).ok();
            }
        }
    }
    ...
    let compose_cmd = halvor_docker::get_compose_command(exec)?;
...
pub fn install_agent<E: CommandExecutor>(exec: &E) -> Result<()> {
    ...
    halvor_docker::ensure_docker_running(exec)?;
