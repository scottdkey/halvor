...
pub fn verify_vpn(hostname: &str, config: &crate::config::EnvConfig) -> Result<()> {
    ...
    if openvpn_check.status.success() {
        let pid = String::from_utf8_lossy(&openvpn_check.stdout).trim().to_string();
    ...
    if privoxy_check.status.success() {
        let pid = String::from_utf8_lossy(&privoxy_check.stdout).trim().to_string();
    ...
    let error_output = String::from_utf8_lossy(&error_check.stdout);
    if error_output.contains("No errors found") || error_output.trim().is_empty() {
        println!("   ✓ No recent errors in OpenVPN logs");
    } else {
        println!("   ⚠ Found potential issues in logs:");
        for line in error_output.lines().take(5) {
